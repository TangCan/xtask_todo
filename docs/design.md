...
- **β（宿主库侧）**：**`--features beta-vm`**，**`session_beta::BetaSession`** 经 **`DEVSHELL_VM_SOCKET`** 连接侧车（Unix：**UDS** / **`tcp:`**；**Windows**：默认 **`stdio`**，不经由本机监听端口）。**宿主工作区根**由 **`workspace_host::workspace_parent_for_instance`** 解析（顺序见 **requirements §5.2**）。**`session_start`** 将 **`staging_dir`**（与 **`guest_workspace`**，默认 **`/workspace`**）发给侧车，后续 **`guest_fs`** / **`exec`** 带同一 **`session_id`**；**`exec`** 可按 **`exec_timeout_ms_from_env`**（**`DEVSHELL_VM_EXEC_TIMEOUT_MS`**）附带 **`timeout_ms`**。
...
- **`exec` 超时**：**`exec`** 请求可带 **`timeout_ms`**；宿主 **`session_beta`** 可按 **`ENV_DEVSHELL_VM_EXEC_TIMEOUT_MS`** 注入；侧车亦可读同名环境变量作为默认。超时时返回错误帧，**`code`** 为 **`exec_timeout`**。**Unix** 侧车为子进程 **`process_group(0)`**，超时 **`kill(-pgid)`** 以终止 **`sh -c …`** 子树（见 **`crates/devshell-vm/src/server.rs`**、**[test-cases.md](./test-cases.md) TC-D-VM-7**）。
