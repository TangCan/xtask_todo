...
- **非 Unix（Windows）**：**`vm_workspace_host_root`** 等对 Lima 宿主路径的 API 在 **`#[cfg(unix)]`** 下实现；**`#[cfg(not(unix))]`** 提供**桩**实现，因 **`devshell/mod.rs`** 中 **`if cfg!(unix) && … { vm_workspace_host_root() }`** 分支在非 Unix 上仍参与**类型检查**。**`WorkspaceBackendError`** 在 **`command/dispatch.rs`** 中**始终**可解析（不限于 **`cfg(unix)` import**），以支持 **`WorkspaceReadError::Backend`** 的映射。
...
- **`acceptance`**：编排 **`cargo test`**（**xtask-todo-lib** / **xtask** / **devshell-vm**）、**NF-1/2/6** 文件检查、**Windows MSVC 交叉 `cargo check`**，**写 `docs/acceptance-report.md`**；实现见 **`xtask/src/acceptance.rs`**。
