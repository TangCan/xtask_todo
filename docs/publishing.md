...
本仓库为 Cargo workspace，仅 **`crates/todo`** 适合发布到 [crates.io](https://crates.io)；**xtask** 已设置 `publish = false`，仅作为工作区内部工具使用，不发布。

**注意**：crates.io 上若已有同名包 `todo`，需在 `crates/todo/Cargo.toml` 中将 `name` 改为未占用的名称（如 `xtask-todo-lib`），再发布。
...
repository = "https://github.com/你的用户名/xtask_todo"  # 替换为实际仓库
# homepage = "https://..."
...
- 在项目根执行：`cargo publish -p todo --dry-run`（若使用镜像 registry，可加 `--registry crates-io`）。
...
- 会打包 `todo` crate 并上传到 crates.io。
- 若未登录或 Token 无效，会提示先执行 `cargo login`。

### 2.3 发布后

- 页面：`https://crates.io/crates/todo`（若包名为 `todo`；若名字已被占用，需在 `Cargo.toml` 中改用其他 `name`）。
- 其他项目可通过在 `Cargo.toml` 中写 `todo = "0.1"` 依赖并 `cargo build` 使用。
...
| 目标                 | 操作 |
|----------------------|------|
| 发布 **todo** 到 crates.io | 补全 `crates/todo` 的 license 等元数据 → `cargo login` → `cargo publish -p todo` |
| **xtask** 不发布     | 已设置 `publish = false`，无需改动 |
| 仅 Git tag / Release | 给仓库打 tag，在托管平台上创建 Release，可不执行 `cargo publish` |

如有 CI（如 GitHub Actions），可在工作流中增加：在 tag 推送时自动执行 `cargo publish -p todo`（需在 CI 中配置 `CARGO_REGISTRY_TOKEN` 等），实现发布自动化。
