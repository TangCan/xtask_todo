...
fn builtin_ls_lists_directory() {
    ...
    assert!(
        out.contains("bar"),
        "ls output should contain 'bar'; got {out:?}"
    );
}
...
fn builtin_mkdir_creates_directory() {
    ...
    assert!(
        ...
        "ls / should contain 'baz'; got {names:?}"
    );
}
...
fn builtin_echo_writes_args_joined_by_space() {
    let mut vfs = Vfs::new();
    let (stdout, _, result) =
        run_builtin_with_buffers(&mut vfs, vec!["echo", "hello", "world"], vec![]);
    result.unwrap();
    let out = String::from_utf8(stdout).unwrap();
...
fn builtin_help_lists_commands() {
    ...
    let out = String::from_utf8(stdout).unwrap();
    assert!(
        out.contains("Supported commands"),
        "help should list header; got {out:?}"
    );
...
fn builtin_unknown_command_returns_err_and_writes_stderr() {
    ...
    assert!(
        err_out.contains("unknown command"),
        "stderr should mention unknown command; got {err_out:?}"
    );
}
...
fn builtin_export_readonly_copies_vfs_to_temp_dir() {
    ...
    let (stdout, _, result) =
        run_builtin_with_buffers(&mut vfs, vec!["export-readonly", "/"], vec![]);
    result.unwrap();
    let out = String::from_utf8(stdout).unwrap();
    ...
    assert!(
        ...
        "export dir should exist: {host_path}"
    );
