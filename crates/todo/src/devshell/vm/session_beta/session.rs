...
use super::super::{
    config::{
        exec_timeout_ms_from_env, ENV_DEVSHELL_VM_BETA_SESSION_STAGING, ENV_DEVSHELL_VM_SOCKET,
    },
    guest_fs_ops::{validate_guest_path_under_mount, GuestFsError, GuestFsOps},
    sync::{pull_workspace_to_vfs, push_incremental},
    workspace_host::{guest_dir_for_vfs_cwd, workspace_parent_for_instance},
    VmConfig, VmError, VmExecutionSession, WorkspaceMode,
};
...
use super::ipc::{
    connect_ipc, exit_status_from_code, parse_devshell_vm_socket, IpcStream, SocketSpec,
};
...
impl BetaSession {
            ...
            .unwrap_or_else(|| {
                #[cfg(all(windows, feature = "beta-vm"))]
                {
                    if let SocketSpec::Stdio = &spec {
                        return super::super::podman_machine::stdio_guest_mount(&workspace_parent);
                    }
                }
                "/workspace".to_string()
            });
    ...
    fn conn(&mut self) -> Result<&mut IpcStream, VmError> {
        if self.stream.is_none() {
            #[cfg(all(windows, feature = "beta-vm"))]
            {
                if std::env::var(super::super::config::ENV_DEVSHELL_VM_SKIP_PODMAN_BOOTSTRAP)
                    .is_err()
                {
                    if let SocketSpec::Stdio = &self.spec {
                        super::super::podman_machine::ensure(&self.workspace_parent)?;
                    }
                }
            }
